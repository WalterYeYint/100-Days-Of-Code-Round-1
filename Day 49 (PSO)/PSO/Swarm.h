...
const float A = 0.0f; //Koeffizient A beeinflusst die Motivation für die aktuelle Geschwindigkeit
const float B = 0.0f; //Koeffizient B beeinflusst die Motivation für Richtung zur besten beobachteten Position
...
const float U = 1.0; //untere Schranke von rt (beeinflusst C)
const float T = 2.0; //obere Schranke von rt
...
public:
	...
	void Initialize( int width, int height )
	{
		...
		for (int i = 0; i < PARTICLES_COUNT; ++i)
		{
			...
			particle._velocity = glm::vec3(getRandomNumberFloat(-1.0f, 1.0f), getRandomNumberFloat(-1.0f, 1.0f), 0.0f);
